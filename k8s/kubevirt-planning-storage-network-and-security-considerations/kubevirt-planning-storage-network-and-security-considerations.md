<!--
title: KubeVirt规划核心：存储、网络与安全实践指南
cover: https://cdn.thenewstack.io/media/2025/12/5c24c3d4-kubevirt-planning-storage-network-security.jpg
summary: KubeVirt平台需规划存储（利用Kubernetes PVC）、网络（默认Pod网，Multus多网）及安全（继承Kubernetes命名空间、RBAC、网络策略）。与Kubernetes深度集成，统一管理虚拟机。
-->

KubeVirt平台需规划存储（利用Kubernetes PVC）、网络（默认Pod网，Multus多网）及安全（继承Kubernetes命名空间、RBAC、网络策略）。与Kubernetes深度集成，统一管理虚拟机。

> 译自：[KubeVirt Planning: Storage, Network and Security Considerations](https://thenewstack.io/kubevirt-planning-storage-network-and-security-considerations/)
> 
> 作者：Janakiram MSV
