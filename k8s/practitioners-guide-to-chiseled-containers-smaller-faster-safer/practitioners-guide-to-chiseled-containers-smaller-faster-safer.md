...
summary: 凿刻容器是精简镜像，只含运行应用必要组件，提升安全性、性能和操作简便性。适用于受监管、电商及边缘场景，是未来安全部署趋势。
-->

凿刻容器是精简镜像，只含运行应用必要组件，提升安全性、性能和操作简便性。适用于受监管、电商及边缘场景，是未来安全部署趋势。

> 译自：[Practitioners’ Guide to Chiseled Containers: Smaller, Faster, Safer](https://thenewstack.io/practitioners-guide-to-chiseled-containers-smaller-faster-safer/)
> 
> 作者：Pankaj Gupta
...
此[演示](https://youtu.be/CNOmqaoJYys)展示了在VMware Cloud Foundation (VCF)上使用Canonical凿刻Ubuntu容器的优势。
