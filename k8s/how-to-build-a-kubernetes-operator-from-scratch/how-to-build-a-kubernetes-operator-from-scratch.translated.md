# 如何从零开始构建 Kubernetes Operator

![从零开始构建 Kubernetes Operator 的特色图片](https://cdn.thenewstack.io/media/2025/05/bc9c55e1-build-kubernetes-operator-1024x576.jpg)
...
*   **完整的本地工作流程：** 学习如何使用 [Kind](https://kind.sigs.k8s.io/)（一种用于在本地测试 Kubernetes 的工具）和容器管理器 Docker 在本地运行和测试所有内容。
*   **真实世界的 operator 模式：** 你将创建自定义资源定义 (CRD)，实现协调循环，并利用 Kubernetes 客户端 API。
*   **即时反馈：** 然后，你将测试你的 operator，并观察它自动将崩溃信息实时注释到 Pod。
...
- 检查 pod 是否与任何 PodDiagnoser 的目标标签匹配。
- 检查容器重启信息。
- 如果检测到崩溃，则添加诊断注释。
- 应用任何配置的修复措施。
...
213
```
Okay, I understand. Please provide the markdown text you would like me to correct. I will only output the corrected markdown, without any explanations.
更新基于角色的访问控制 (RBAC) 清单：
...
- 找到你的 Operator 的 Deployment YAML 文件，通常在 `config/manager/manager.yaml` 中
- 添加 `imagePullPolicy: Never` 到容器规范中：
