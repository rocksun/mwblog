...
summary: Cover Whale在构建内部开发者平台时，从MVP到扩展面临Helm蔓延、非Kubernetes集成挑战。引入Kratix解决复杂性，实现声明式NATS支持和秘密管理。教训：警惕Helm，规划SaaS集成，测试秘密管理。
-->

Cover Whale在构建内部开发者平台时，从MVP到扩展面临Helm蔓延、非Kubernetes集成挑战。引入Kratix解决复杂性，实现声明式NATS支持和秘密管理。教训：警惕Helm，规划SaaS集成，测试秘密管理。

> 译自：[How Cover Whale Scaled Its Developer Platform Beyond an MVP](https://thenewstack.io/how-cover-whale-scaled-its-developer-platform-beyond-an-mvp/)
> 
> 作者：Laurent Sibilla
