...
summary: TrapC提案旨在为C语言引入内存安全扩展，兼容现有C代码，甚至C++。通过*trap*关键字提供安全错误处理，*alias*方便重载。TrapC编译器利用AI代码推理优化指针检查，提升性能。目标是使C/C++代码编译后具备防缓冲区溢出和崩溃能力，并提供类型安全的*printf*和*scanf*。
-->
