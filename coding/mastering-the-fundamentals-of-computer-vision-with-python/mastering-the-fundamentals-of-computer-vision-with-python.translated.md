# 使用 Python 掌握计算机视觉基础知识

![使用 Python 掌握计算机视觉基础知识的特色图片](https://cdn.thenewstack.io/media/2024/08/4129f28e-computervision-1024x576.jpg)
...
* **已安装 Python：**本文使用 Python 3.12.4。你可以通过运行以下命令检查你的 Python 版本：
  ```bash
  python --version
  ```
  如果遇到错误，请确保 Python 正确安装。你可以从 [官方网站](https://www.python.org/downloads/) 下载 Python。

* **文本编辑器：**本教程使用 Visual Studio Code (VS Code) 作为文本编辑器。你可以 [在这里](https://code.visualstudio.com/) 下载 VS Code。但是，你可以随意使用你选择的任何文本编辑器。

在深入我们的项目之前，必须设置一个干净的工作环境。以下是逐步操作方法：

**创建项目文件夹：**首先，选择项目文件夹的位置。在本教程中，我们将在桌面上创建它。
...
**创建并激活虚拟环境：**这有助于将项目依赖项与全局 Python 安装隔离开来。
...
* 激活虚拟环境：
    * 要激活虚拟环境，请根据你的操作系统使用以下命令：
      ```bash
      source venv/bin/activate //在 mac 上激活虚拟环境
...
# 加载参考图像
known_image = face_recognition.load_image_file("images/raymond.jpg")
# 对参考图像进行编码
    ...
    while True:
        ...
        try:
            ...
            for (x, y, w, h) in faces:
                ...
                # 对检测到的人脸进行编码
                encoded_face = face_recognition.face_encodings(face_image)
                # 检查是否检测到人脸
                if len(encoded_face) > 0:
                    # 比较编码的人脸
                    match = face_recognition.compare_faces([known_face_encoding], encoded_face[0])
                    # 如果匹配，则显示姓名
                    if match[0]:
                        cv2.putText(frame, "Raymond", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            # 显示带有面部检测的帧
            cv2.imshow("人脸检测", frame)
        except cv2.error as e:
            print(f"OpenCV 错误：{e}")
...
现在，当您运行代码时，如果检测到您的脸并且与参考图像匹配，则您的姓名“Raymond”将显示在边界框旁边。
```python
pip install face_recognition
...
def accessWebcam(): # Access the webcam
    cap = cv2.VideoCapture(0)
    if not cap.isOpened():
...
If you encounter a runtime error `RuntimeError: Unsupported image type, must be 8bit gray or RGB image.`, you can resolve it by downgrading NumPy to version 1.26.4. This issue is related to NumPy 2.0. To downgrade, simply run the following command:
`pip install numpy==1.26.4`

Now, let me demonstrate the fruits of our labor. You'll notice that when my face is recognized, the bounding box turns green, and my name is displayed next to it. If the face is not recognized, the bounding box will remain blue. Additionally, I tested the system by showcasing a second face from my phone. While the model detected the face, it didn't recognize it, so that face's bounding box remained blue, while my face was highlighted with a green bounding box due to the model's familiarity with it.

![Figure 3: Illustration showing the detection and recognition of my face](https://cdn.thenewstack.io/media/2024/08/f3b2addd-image5-1024x640.png)
Figure 3: Illustration showing the detection and recognition of my face

![Figure 4: Illustration showing a face detected but not recognized due to head tilt](https://cdn.thenewstack.io/media/2024/08/7dfa9115-image3-1024x640.png)
Figure 4: Illustration showing a face detected but not recognized due to head tilt

![Figure 5: Illustration showing the detection of two faces, with only my face being recognized](https://cdn.thenewstack.io/media/2024/08/b4801bbe-image2-1024x640.png)
Figure 5: Illustration showing the detection of two faces, with only my face being recognized

Computer vision presents a wealth of opportunities for innovation and problem-solving, enabling us to [create applications capable of interpreting and responding to visual information in meaningful ways](https://thenewstack.io/apache-hop-harnesses-metadata-to-create-visual-data-pipelines/). From enhancing security through facial recognition to enabling autonomous systems, the possibilities are boundless and exciting. As technology continues to advance, the integration of computer vision across various domains will undoubtedly lead to groundbreaking developments and novel applications. I encourage you to explore and experiment with these technologies further, as they hold the promise of transforming how we interact with the world around us.
了解如何找到熟练的 Python 开发人员，帮助您推动数字化转型，请参阅“[如何招聘 Python 开发人员：找到合适人选的指南](https://www.andela.com/blog-posts/how-to-hire-a-python-developer-a-guide-to-finding-the-right-fit/?utm_medium=contentmarketing&utm_source=blog&utm_campaign=brand-global-the-new-stack-computer-vision&utm_content=hire-python-developers)”。
