...
This problem was previously addressed in the Istio blog post [Routing egress traffic to wildcard destinations](https://istio.io/latest/blog/2023/egress-sni/). The architecture included a dedicated egress gateway setup that worked as an SNI forward proxy.

[![Egress SNI routing with arbitrary domain names](https://istio.io/latest/blog/2026/egress-dynamic-dns/egress-sni-flow.svg)](https://istio.io/latest/blog/2026/egress-dynamic-dns/egress-sni-flow.svg "Egress SNI routing with arbitrary domain names")

Application → sidecar → egress gateway → SNI inspection → external destination

The diagram above was originally published in [Routing egress traffic to wildcard destinations](https://istio.io/latest/blog/2023/egress-sni/).

As shown above:

1. The application initiates an HTTPS connection.
2. The sidecar proxy intercepts this connection and initiates an internal mTLS connection to the egress gateway.
3. The gateway terminates this internal mTLS connection.
4. An internal listener inspects the SNI value from the original TLS handshake.
5. Traffic is dynamically forwarded to the hostname extracted from SNI.
...
[![Wildcard ServiceEntry with DYNAMIC_DNS resolution](https://istio.io/latest/blog/2026/egress-dynamic-dns/egress-dynamic-dns.svg)](https://istio.io/latest/blog/2026/egress-dynamic-dns/egress-dynamic-dns.svg "Wildcard ServiceEntry with DYNAMIC_DNS resolution")

Application → sidecar → external destination
