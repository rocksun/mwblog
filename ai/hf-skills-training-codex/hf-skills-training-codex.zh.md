...
[![banner](https://huggingface.co/blog/assets/hf-skills-training/thumbnail-codex.png)](https://huggingface.co/blog/assets/hf-skills-training/thumbnail-codex.png)
...
```
# Base Model & Dataset
[基础模型](https://huggingface.co/Qwen/Qwen3-0.6B)  
[数据集](https://huggingface.co/datasets/open-r1/codeforces-cots)
...
| 方法 | SFT (TRL) |
| 模型 | `Qwen/Qwen3-0.6B` |
| 数据集 | `open-r1/codeforces-cots` (训练集，5% 评估拆分) |
| 最大长度 | 2048 |
| 轮次 | 1 (首次检查后扩展到 3) |
| 每设备批量大小 | 1 |
| 梯度累积步数 | 8 |
...
| 预热比例 | 0.03 |
| 评估策略 | 步数 (500) |
| 保存策略 | 步数 (500)，`hub_strategy=every_save`，限制=2 |
| 精度 | bf16 |
| 梯度检查点 | true |
| 打包 | false |
| Hub 模型 | `burtenshaw/qwen3-codeforces-cots-sft` |
| 硬件 | a10g-small |
| 超时 | 2h |
| Trackio | 项目 `qwen3-codeforces-cots`，运行 `sft-a10g` |
...
## Run Evaluations
等待中（lighteval `openai_humaneval` 用于基础模型 + 检查点）

---

# Experiment Evaluations
| 运行标题 | 基准 | 分数 | 评估作业链接 | 模型链接 |
|-----------|-----------|-------|---------------------|------------|
| `sft-a10g` - `待定` - `进行中` | HumanEval pass@1 | TBD | TBD | [burtenshaw/qwen3-codeforces-cots-sft](https://huggingface.co/burtenshaw/qwen3-codeforces-cots-sft)
...
```
我将使用 SFT 对 Qwen/Qwen3-0.6B 在 open-r1/codeforces-cots 上进行微调。
...
• 训练和评估作业正在运行。主要细节：
  ...
  - SFT 已在 open-r1/codeforces-cots (solutions_py_decontaminated, 98/2 拆分) 上启动，使用 LoRA 对 Qwen/Qwen3-0.6B 进行训练，硬件为 a10g-
    large，精度 bf16，max_steps=1000，batch=1，grad_accum=8，lr=2e-5，每 200 步保存/评估，推送到 Hub burtenshaw/qwen3-
    codeforces-cots-sft。作业：https://huggingface.co/jobs/burtenshaw/6938272ec67c9f186cfe1ae3 (Trackio 项目 codeforces-
    cots-sft，运行 qwen3-0.6b-lora-v1)。
  - 基准 lighteval (HumanEval) 在 a10g-large 上排队的基础模型。作业：https://huggingface.co/jobs/
  - 实验报告已使用当前运行和链接更新：training_reports/qwen3-codeforces-cots-sft.md。

  一旦 SFT 作业推送到 Hub，要评估微调过的检查点，请重用相同的 lighteval 脚本：
  ...
  （或将 MODEL_ID 替换为特定的检查点路径）。这将记录到 Hub 上的相同结果目录。

  1. 监控这两个正在运行的作业；我可以根据请求获取日志或指标。
...
```
| 模型 | 基准 | 分数 | 评估作业链接 | 模型链接 |
|-----------|-----------|-------|---------------------|------------|
| `qwen3-0.6b-lora-v1` - `2025-12-09 13:47:47 UTC` - `已完成` | HumanEval pass@1 | 0.342 | [日志](<训练作业链接>) | [burtenshaw/qwen3-codeforces-cots-sft](https://huggingface.co/burtenshaw/qwen3-codeforces-cots-sft)
| `base-humaneval-a10g` - `2025-12-09 13:47:47 UTC` - `已完成` | HumanEval pass@1 | 0.306 | [日志](<评估作业链接>) | [Qwen/Qwen3-0.6B](https://huggingface.co/Qwen/Qwen3-0.6B)

```

你还可以实时监控训练损失。

[![Sweep 测试的 Trackio 仪表板示例](https://huggingface.co/datasets/hf-skills/images/resolve/main/codex-sft-codeforces.png)](https://huggingface.co/datasets/hf-skills/images/resolve/main/codex-sft-codeforces.png)
