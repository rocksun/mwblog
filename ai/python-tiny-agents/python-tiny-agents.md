...
summary: 70行代码搞定Python Tiny Agent！基于`huggingface_hub`扩展，通过`MCP`协议连接`LLM`与外部工具，无需定制集成。通过`stdio`、`http`等服务器连接，实现文件系统访问和远程工具调用。核心是`MCPClient`，简化Agent构建，快来体验！
-->

70行代码搞定Python Tiny Agent！基于`huggingface_hub`扩展，通过`MCP`协议连接`LLM`与外部工具，无需定制集成。通过`stdio`、`http`等服务器连接，实现文件系统访问和远程工具调用。核心是`MCPClient`，简化Agent构建，快来体验！

> 译自：[Tiny Agents in Python: a MCP-powered agent in ~70 lines of code](https://huggingface.co/blog/python-tiny-agents)
> 
> 作者：None
