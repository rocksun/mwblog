...
summary: OpenAI发布 Agents SDK 和 Responses API，简化Agent编排。新增 Web search、File Search 和 Computer Use 工具，支持向量存储和模拟用户操作。通过 handoff 实现多Agent协作，解决异步任务。鼓励开发者使用高级API进行实验，降低编排复杂度，未来将深入研究SDK更多功能。
-->
