# 使用 Thanos 和 Kubernetes 构建指标系统

指标是任何分布式系统中可观察性的基石，在 Kubernetes 环境中，Prometheus 通常是首选工具。
...
在此设置中，`dnssrv`
地址用于自动发现 Kubernetes 集群中运行的任何 Prometheus 实例和 Thanos Store Gateway。通过利用 Kubernetes 基于 DNS 的服务发现，Thanos 可以动态扩展，而无需在添加新的 Prometheus 实例时进行任何手动配置。
...
# 结论
使用 **Thanos** 和 **Kubernetes** 设置指标系统可能感觉是一项艰巨的任务，尤其是在您开始考虑长期存储、高可用性和跨多个集群的全局查询时。但好消息是，它并不一定是一个令人生畏的过程。通过将 Thanos 与 Prometheus 集成，您可以解决 Kubernetes 环境中可观察性方面的一些最大挑战——可扩展性、冗余性和成本管理。
...
# 了解更多
## 2024 年 15 款最佳 Kubernetes 成本优化工具
### 财务状况艰难，这是不言而喻的。大多数组织都需要根据其市场规模调整其产品……
overcast.blog

## 优化 Kubernetes 负载均衡的 11 种方法
### 优化 Kubernetes 中的负载均衡对于维护高可用性、可扩展性和性能至关重要……
overcast.blog

## 您应该了解的 15 种云原生 DevOps 工具
### 有效管理动态且可扩展的环境需要云原生 DevOps 工具。这些工具自动化……
overcast.blog