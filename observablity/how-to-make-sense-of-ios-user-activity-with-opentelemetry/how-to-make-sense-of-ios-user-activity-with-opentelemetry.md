...
summary: 用`OpenTelemetry (OTel)`搞定iOS用户行为！通过`metrics`、`logs`、`traces`标准化移动端`observability`。告别单体应用，拥抱模块化，用`Span`在模块间传递遥测数据，无痛集成`Context`，实时监控`UI`、网络、用户认证，让你的App性能一览无余！
-->
