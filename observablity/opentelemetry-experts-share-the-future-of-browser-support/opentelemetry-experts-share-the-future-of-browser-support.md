...
summary: OpenTelemetry在前端观测面临挑战，现有JS SDK主要为Node.js设计。浏览器SIG正致力于优化API和数据模型，定义会话概念。开发者可使用现有JS SDK或演示应用入门。
-->

OpenTelemetry在前端观测面临挑战，现有JS SDK主要为Node.js设计。浏览器SIG正致力于优化API和数据模型，定义会话概念。开发者可使用现有JS SDK或演示应用入门。

> 译自：[OpenTelemetry Experts Share the Future of Browser Support](https://thenewstack.io/opentelemetry-experts-share-the-future-of-browser-support/)
> 
> 作者：Colin Contreary
