...
summary: Talos Linux是为Kubernetes设计的非传统发行版，简化管理。本教程指导Mac用户通过Homebrew安装Talos Linux集群，包括依赖安装、初始化K8s控制面及配置。
-->

Talos Linux是为Kubernetes设计的非传统发行版，简化管理。本教程指导Mac用户通过Homebrew安装Talos Linux集群，包括依赖安装、初始化K8s控制面及配置。

> 译自：[Set Up Talos Linux on Your Machine](https://thenewstack.io/set-up-talos-linux-on-your-machine/)
> 
> 作者：B. Cameron Gain
